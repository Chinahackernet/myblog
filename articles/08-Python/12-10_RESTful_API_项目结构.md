...
> 分类：Python / 第12章：FastAPI
> 原文：https://www.cuiliangblog.cn/detail/section/245282498
> 来源：崔亮的博客
...
# 为什么需要项目结构设计
FastAPI 非常灵活，初学时你可以把所有内容放在一个main.py文件中：
...
将 API 接口按版本号区分（如 `/v1/`、`/v2/`）是一种非常常见且推荐的做法，在实际的生产环境中被广泛采用，目的是：
...
1. 1. 拆分 models / routers / services；
2. 2. 引入数据库 session、CRUD 层；
3. 3. 添加统一异常处理和依赖注入；
4. 4. 最终形成清晰的模块分层。
