...
ssl\_exporter就是来做这个事情的。ssh\_exporter是一个Prometheus Exporter能提供多种针对 SSL 的检测手段，包括：https 证书生效/失效时间、文件证书生效/失效时间，OCSP 等相关指标。
...
现在可以在prometheus的web界面看到正常的抓取任务了，如下：![image.png](assets/kubernetes与云原生/使用ssl_exporter监控K8S集群证书/使用ssl_exporter监控K8S集群证书-1.png)

  

然后通过`(ssl_cert_not_after-time())/3600/24`即可看到证书还有多久失效。

![image.png](assets/kubernetes与云原生/使用ssl_exporter监控K8S集群证书/使用ssl_exporter监控K8S集群证书-2.png)

  

通过`ssl_tls_connect_success`可以观测ssl链接是否正常。

![image.png](assets/kubernetes与云原生/使用ssl_exporter监控K8S集群证书/使用ssl_exporter监控K8S集群证书-3.png)

  

## 告警

上面已经安装ssl\_exporter成功，并且能正常监控数据了，下面就配置一些告警规则，以便于运维能快速知道这个事情。
...
![image.png](assets/kubernetes与云原生/使用ssl_exporter监控K8S集群证书/使用ssl_exporter监控K8S集群证书-4.png)