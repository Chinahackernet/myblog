...
![image.png](assets/kubernetes与云原生/7.10、argocd使用优化/7.10、argocd使用优化-1.png)

由于集群内部证书是无效证书，所有要把Enabled SSL去掉，如下：

![image.png](assets/kubernetes与云原生/7.10、argocd使用优化/7.10、argocd使用优化-2.png)

然后点击保存，点击测试，看是否链接成功。如果有如下提示则表示webhook配置没问题了。

![image.png](assets/kubernetes与云原生/7.10、argocd使用优化/7.10、argocd使用优化-3.png)
...
![image.png](assets/kubernetes与云原生/7.10、argocd使用优化/7.10、argocd使用优化-4.png)

  

然后在Grafana上导入以下[json](https://raw.githubusercontent.com/argoproj/argo-cd/master/examples/dashboard.json)内容，即可在面板查看信息：

![image.png](assets/kubernetes与云原生/7.10、argocd使用优化/7.10、argocd使用优化-5.png)