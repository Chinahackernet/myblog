# 其他容器命令

> 分类：Docker / 第3章：容器
> 原文：https://www.cuiliangblog.cn/detail/section/26457075
> 来源：崔亮的博客

---

1. 复制文件  
[root[@docker ](/docker ) ~]# docker cp centos.tar 8e3b24402577:/ 
2. 查看容器文件系统的变更  
[root[@docker ](/docker ) ~]# docker diff 8e3b24402577 
3. 查看端口映射  
[root[@docker ](/docker ) ~]# docker port 8e3b24402577 
4. 更新容器运行时的配置，主要是资源限制  
[root[@docker ](/docker ) ~]# docker update --cpu-period 100000 8e3b24402577 
5. 容器直接互联
+ 启动mysql容器  
[root[@docker ](/docker ) ~]# docker run --name mysqldb -p 3306:3306  -e MYSQL_ROOT_PASSWORD=CHANGE_ME -d mysql:latest 
+ 启动httpd容器与mysql容器互联  
[root[@docker ](/docker ) ~]# docker run -it --name web --link mysqldb:webdb httpd /bin/bash 
+ 测试链接

![](assets/11-Docker/b789783079897c456d2a.png)


