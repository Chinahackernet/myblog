...
Django settings.py配置文件中默认没有 APPEND\_SLASH 这个参数，但 Django 默认这个参数为 APPEND\_SLASH = True。 其作用就是自动在网址结尾加'/'。
...
如果在settings.py中设置了 **APPEND\_SLASH=False**，此时我们再请求 http://www.example.com/blog 时就会提示找不到页面。
...
views.month\_archive(request, year="2017", month="12")
...
url(r'^articles/(?P<year>[0-9]{4})/$', views.year\_archive),
...
在这个例子中，对于/blog/2005/请求，Django 将调用views.year\_archive(request, year='2005', foo='bar')。
...
-   在更高层的与处理Django 模型实例相关的代码中：使用get\_absolute\_url() 方法。
