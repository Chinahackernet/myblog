...
> 原文：https://www.cuiliangblog.cn/detail/section/162609657
> 来源：崔亮的博客

---

> es可以使用二进制、docker、k8s、rpm方式部署，此处以rpm方式为例。相较于二进制部署，省去了繁琐的用户创建、证书生成、密码设置、启动脚本配置等操作，简化部署流程，可以将更多的精力用于es的使用而不是部署上面。如果资源有限，想体验elk相关功能，可参考文档：[01-09_ES%E4%B8%8Ekibana%E9%83%A8%E7%BD%B2%28docker%29.md](articles/01-ELK-Stack/01-09_ES与kibana部署-docker-.md)，后续也会发布docker模式下elk自定义日志采集文章。
>
...
## <font style="color:rgb(48, 49, 51);">关闭防火墙和selinux</font>
```plain
...
## <font style="color:rgb(48, 49, 51);">系统参数优化</font>
### <font style="color:rgb(48, 49, 51);">修改文件描述符数目</font>
+ **<font style="color:rgb(48, 49, 51);">修改原因</font>**

<font style="color:rgb(48, 49, 51);">原因1： Elasticsearch 在节点和 HTTP 客户端之间进行通信也使用了大量的套接字（sockets）。 所有这一切都需要足够的文件描述符。  
</font><font style="color:rgb(48, 49, 51);">原因2：linux系统对每个用户、进程、或整个系统的可打开文件描述符数量都有一个限制，一般默认为1024。这对一个小的 Elasticsearch 节点来说实在是太低了，更不用说一个处理数以百计索引的节点。</font>
...
<font style="color:rgb(48, 49, 51);">Elasticsearch 对各种文件混合使用了 NioFs（ 非阻塞文件系统）和 MMapFs （ 内存映射文件系统）。  
</font><font style="color:rgb(48, 49, 51);">请确保配置的</font>**<font style="color:rgb(48, 49, 51);">最大映射数量</font>**<font style="color:rgb(48, 49, 51);">，以便有足够的虚拟内存可用于 mmapped 文件</font>
...
+ **<font style="color:rgb(48, 49, 51);">设置</font>**
...
> 具体参考文章：[../10_Linux/07-06_fdisk%E5%88%86%E5%8C%BA-%E5%88%86%E5%8C%BA%E8%BF%87%E7%A8%8B.md](articles/10-Linux/07-06_fdisk分区-分区过程.md)
>
...
## 下载rpm包
国内下载地址：[https://elasticsearch.cn/download/](https://elasticsearch.cn/download/)

官网下载地址：[https://www.elastic.co/cn/downloads/past-releases#elasticsearch](https://www.elastic.co/cn/downloads/past-releases#elasticsearch)

```plain
wget https://artifacts.elastic.co/downloads/elasticsearch/elasticsearch-8.8.2-x86_64.rpm
```

## <font style="color:rgb(48, 49, 51);">安装elasticsearch</font>
...
## <font style="color:rgb(48, 49, 51);">修改es配置</font>
...
<font style="color:rgb(48, 49, 51);">启动master节点服务并生成token</font>
...
<font style="color:rgb(77, 77, 77);">生成集群注册token</font>
...
<font style="color:rgb(77, 77, 77);">使用默认配置文件执行加入集群，token为上面master节点生成的</font>
...
```bash
[root@es-master ~]# curl -X PUT -H 'content-type:application/json' -d '{"transient":{"cluster.routing.allocation.exclude._ip":"192.168.10.132"}}' --cacert /etc/elasticsearch/certs/http_ca.crt https://elastic:CHANGE_ME@127.0.0.1:9200/_cluster/settings
...
```bash
[root@es-master ~]# curl --cacert /etc/elasticsearch/certs/http_ca.crt https://elastic:CHANGE_ME@127.0.0.1:9200/_cat/shards?
...
```bash
[root@es-master ~]# curl --cacert /etc/elasticsearch/certs/http_ca.crt https://elastic:CHANGE_ME@127.0.0.1:9200/_cat/health?
...
```bash
[root@es-master ~]# curl --cacert /etc/elasticsearch/certs/http_ca.crt https://elastic:CHANGE_ME@127.0.0.1:9200/_cat/nodes?v
...
# <font style="color:rgb(48, 49, 51);">kibana安装配置</font>
> 以下操作在es-master节点执行
>
...
## <font style="color:rgb(48, 49, 51);">访问验证</font>
+ <font style="color:rgb(48, 49, 51);">登录kibana </font>http://192.168.10.132:5601，用户名为elasic，密码为上述步骤中重置的elastic用户密码

![](assets/01-ELK-Stack/020e1b6162c552a6f227.png)

## 导入样例数据
登录进kibana后，我们选择使用样例数据

![](assets/01-ELK-Stack/faeab3d11c0e7f3bfb36.png)

然后导入kibana提供的样例数据

![](assets/01-ELK-Stack/6eb2b031437efba984f3.png)

## monitor异常处理
访问kibana的monitor时，会提示以下报错。

![](assets/01-ELK-Stack/f073cebcd7c62404442e.png)
...
![](assets/01-ELK-Stack/81bc2f3cf33de34db716.png)

# <font style="color:rgb(48, 49, 51);">官方文档</font>
rpm安装es服务：[https://www.elastic.co/guide/en/elasticsearch/reference/current/rpm.html](https://www.elastic.co/guide/en/elasticsearch/reference/current/rpm.html)

<font style="color:rgb(48, 49, 51);">es配置文件：</font>[<font style="color:rgb(48, 49, 51);">https://www.elastic.co/guide/en/elasticsearch/reference/current/important-settings.html</font>](https://www.elastic.co/guide/en/elasticsearch/reference/current/important-settings.html)

<font style="color:rgb(48, 49, 51);">es节点角色配置：</font>[<font style="color:rgb(48, 49, 51);">https://www.elastic.co/guide/en/elasticsearch/reference/current/modules-node.html#node-roles</font>](https://www.elastic.co/guide/en/elasticsearch/reference/current/modules-node.html#node-roles)

<font style="color:rgb(48, 49, 51);">jvm配置：</font>[<font style="color:rgb(48, 49, 51);">https://www.elastic.co/guide/en/elasticsearch/reference/current/advanced-configuration.html#set-jvm-options</font>](https://www.elastic.co/guide/en/elasticsearch/reference/current/advanced-configuration.html#set-jvm-options)

<font style="color:rgb(48, 49, 51);">kibana配置文件：</font>[<font style="color:rgb(48, 49, 51);">https://www.elastic.co/guide/en/kibana/current/settings.html</font>](https://www.elastic.co/guide/en/kibana/current/settings.html)
