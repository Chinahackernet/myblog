...
> 原文：https://www.cuiliangblog.cn/detail/section/110037432
> 来源：崔亮的博客
...
[root@es-master ~]# docker run -d -p 9000:9000  -p 9090:9090  --name minio  -v /data/minio:/data -e "MINIO_ROOT_USER=root" -e "MINIO_ROOT_PASSWORD=CHANGE_ME --restart always quay.io/minio/minio server /data --console-address ":9090"
```

## 访问测试
访问master节点ip:9090端口

![](assets/01-ELK-Stack/50c3d90f2ced8b685b79.png)

# 创建bucket和Access Key
## 创建bucket
创建一个名为es-backup的bucket，并设置容量上限为1TB

![](assets/01-ELK-Stack/eda07812f3b089ab8877.png)

## 创建Access Key
创建access key并牢记，后续使用。

![](assets/01-ELK-Stack/f8018568cf1874a6a47a.png)
...
![](assets/01-ELK-Stack/071214620b847fbec662.png)
...
![](assets/01-ELK-Stack/6ebb1e058222f495be6b.png)
...
Enter value for s3.client.default.access_key: CHANGE_ME ~]# /usr/share/elasticsearch/bin/elasticsearch-keystore add s3.client.default.secret_key
...
![](assets/01-ELK-Stack/67f3ddc659fa61a471cc.png)

填写存储桶名称为es-backup，其他保持默认即可

![](assets/01-ELK-Stack/18621a09c6071133b6b1.png)

注册完成后点击验证存储库，显示已连接

![](assets/01-ELK-Stack/ef4da9dbc33f8bbc173a.png)
...
![](assets/01-ELK-Stack/c8be784e434151d19627.png)

创建备份策略，名称自定义，快照表达式填写<kibana_sample_data_flights-{now/d}>![](assets/01-ELK-Stack/d64d3532d64b42331945.png)

选择索引为kibana_sample_data_flights

![](assets/01-ELK-Stack/5b67c46bf654fc6b8c0b.png)

设置快照保留策略

![](assets/01-ELK-Stack/d157f9eef3eb8f902579.png)

备份策略创建后，界面如下所示

![](assets/01-ELK-Stack/e5e3ba95e5db7749c5e5.png)

## 数据备份
创建完策略后，es会在每天8点自动将数据上传至minIO备份，我们也可以点击立即执行，手动触发一次备份任务操作。

![](assets/01-ELK-Stack/5fa6a2f237ea4912562a.png)

点击立即执行后，查看快照信息，显示已备份完成。

![](assets/01-ELK-Stack/5c99fed2773d541df726.png)

查看minIO的bucket信息，发现已成功上传数据

![](assets/01-ELK-Stack/c54a07c3c66fbfa51d13.png)

## 数据恢复
接下来删除index，模拟误操作情况发生。

![](assets/01-ELK-Stack/b493f8527bde57c3aa00.png)

在快照菜单点击恢复按钮，执行恢复操作

![](assets/01-ELK-Stack/744449e6533eec3b6caf.png)

使用默认选项还原即可

![](assets/01-ELK-Stack/9c1e9ec6a02d84598b97.png)

查看还原状态显示已完成

![](assets/01-ELK-Stack/8bea470ed06ad0a19138.png)

接下来打开索引管理，发现index已正常还原，且文档数、分片数、存储大小等信息与先前保持一致。

![](assets/01-ELK-Stack/363f7c88d2c7b00ef446.png)
