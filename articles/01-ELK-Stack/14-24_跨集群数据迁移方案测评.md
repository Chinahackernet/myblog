...
> 原文：https://www.cuiliangblog.cn/detail/section/162609161
> 来源：崔亮的博客
...
![](assets/01-ELK-Stack/08c07fd061ef4fcd4415.png)
...
![](assets/01-ELK-Stack/2a68359e854149452a6d.png)
...
![](assets/01-ELK-Stack/072d9b9184bfd50e74f1.png)
...
### 源集群配置
源数据集群配置NFS共享存储库具体可参考文档：[06-11_%E9%9B%86%E7%BE%A4%E5%A4%87%E4%BB%BD%E4%B8%8E%E6%81%A2%E5%A4%8D%28NFS%29.md](articles/01-ELK-Stack/06-11_集群备份与恢复-NFS-.md)，配置完后效果如下所示：

![](assets/01-ELK-Stack/ded391bf3f32c2a65064.png)

接下来配置索引快照存储策略，配置完后后点击立即运行。

![](assets/01-ELK-Stack/fb19aec6b34491440e8a.png)

等待备份完成，查看备份结果，2.6G数据备份耗时78秒

![](assets/01-ELK-Stack/07b3b051db8d85c3c05d.png)

### 目的集群配置
操作与源集群类似，先配置nfs共享存储仓库，配置完后效果如下所示：

![](assets/01-ELK-Stack/9c7bb20f8210c4907216.png)

因为源集群和目的集群使用同一个备份存储库，所以目的集群配置完存储库后就可以加载出源集群的备份数据。

![](assets/01-ELK-Stack/36e6e14b7626f289abc8.png)

在还原过程中通常会设置number_of_replicas: 0和refresh_interval: -1两个参数，以加速数据恢复。

![](assets/01-ELK-Stack/d8c33a958ea8878f59f2.png)

等待还原完成，查看还原结果，2.6G数据还原耗时71秒

![](assets/01-ELK-Stack/b114bc4c21e72ab26f73.png)

恢复后的索引数据如下所示，由于设置了1个副本，因此存储大小变为5.3G

![](assets/01-ELK-Stack/4af9d2f299ec7bc68092.png)
...
--input=http://elastic:CHANGE_ME@192.168.10.70:9200/geonames \
--output=https://elastic:CHANGE_ME@192.168.10.61:9200/geonames \
--type=mapping
...
```bash
[root@tiaoban opt]# ./esm-linux-amd64 -s http://192.168.10.70:9200 -m elastic:CHANGE_ME -x "geonames" -d https://192.168.10.61:9200 -n "elastic:yoVU+Pit0YnVvr+xO_B1" -c 10000 -w10 -b=20 -f --refresh
...
## 迁移方案
CCR方案：ES官方推出了CCR跨集群复制方案，可以完美实现数据的实时增量迁移，具体可参考文档[06-07_%E8%B7%A8%E9%9B%86%E7%BE%A4%E5%A4%8D%E5%88%B6.md](articles/01-ELK-Stack/06-07_跨集群复制.md)，需要注意的是开源社区版不支持该功能。
...
![](assets/01-ELK-Stack/dfb762c54e2b46357f64.png)

es8集群同样可以查询到id为"AYshUY4BV0xJuZCSuWf6"的文档，且内容完全一致。

![](assets/01-ELK-Stack/3e47d2555337d5233f04.png)
