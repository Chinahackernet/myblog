...
> 原文：https://www.cuiliangblog.cn/detail/section/162609519
> 来源：崔亮的博客
...
更多ingest相关的内容，可参考文章：[03-09_%E6%95%B0%E6%8D%AE%E5%A4%84%E7%90%86%EF%BC%88Ingest_Pipeline%EF%BC%89.md](articles/01-ELK-Stack/03-09_数据处理（Ingest_Pipeline）.md)

## 查看es存储数据字段
除了使用discover查看文档json信息外，我们也可以通过dev tools工具查看

![](assets/01-ELK-Stack/bfd8433f5dcf84a963e5.png)
...
![](assets/01-ELK-Stack/ce5c76d955a63c1ceec8.png)
...
![](assets/01-ELK-Stack/6cd86c89cc83ebbbbea9.png)

查看页面运行结果，发现已成功去除对应的字段

![](assets/01-ELK-Stack/edd4dfcfc4d85466f699.png)

接下来点击创建并保存管道即可。

![](assets/01-ELK-Stack/ec72ca0b0de63bcf3acf.png)

# 组件模板设置
## 场景需求
我们查看写入的索引信息可以发现，es默认为这个索引设置了1个分片和1个副本，但我们有3个hot节点，我们期望索引可以有3个分片平均分配到hot节点，分摊节点压力，提高查询和写入速度。

![](assets/01-ELK-Stack/3c54f582d15fbb8045ac.png)
...
<font style="color:rgb(48, 49, 51);">模板有两种类型：索引模板和组件模板。 组件模板是可重用的构建块，用于配置映射，设置和别名。 你使用组件模板来构造索引模板，但它们不会直接应用于一组索引。 索引模板可以包含组件模板的集合，也可以直接指定设置，映射和别名。更多组件模板配置请参考文章：</font>[<font style="color:rgb(48, 49, 51);">03-07_%E7%BB%84%E4%BB%B6%E6%A8%A1%E6%9D%BF%28component_templates%29.md</font>](articles/01-ELK-Stack/03-07_组件模板-component_templates-.md)
...
![](assets/01-ELK-Stack/16e4eb5789f7672de86a.png)
...
![](assets/01-ELK-Stack/5128c15cae7091cfe6e0.png)

映射和别名暂不做设置，然后创建组件模板

![](assets/01-ELK-Stack/5369eff66848f34e29f6.png)

## 配置通用mapping
接下来我们创建一个名为<font style="color:rgb(26, 28, 33);">geoip-mapping的组件模板，只配置mapping相关内容。</font>

![](assets/01-ELK-Stack/e542d57bc3733ae93bad.png)
...
![](assets/01-ELK-Stack/0b476a665b9a77662301.png)

接下来创建这个组件模板

![](assets/01-ELK-Stack/afdb0b8d7d66d2a08f48.png)

# 索引模板设置
创建完采集管道后，接下来我们要使用这个管道，我们可以通过配置索引模板的方式，指定pipeline，后续创建的index都会安装我们设置的模板自动创建并指定pipeline，更多索引模板配置请参考文章：[03-05_%E7%B4%A2%E5%BC%95%E6%A8%A1%E6%9D%BF%EF%BC%88template%EF%BC%89.md](articles/01-ELK-Stack/03-05_索引模板（template）.md)

## 查看索引模板
我们点击索引管理，查看数据流详细信息，就可以看到当前索引配置是由名为logs的模板管理。通常情况下都会根据实际需求单独为每个索引创建新的索引模板。

![](assets/01-ELK-Stack/51bf3b32acf7daeb932f.png)

## 创建索引模板
我们找到默认的logs索引模板，查看模板的优先级

![](assets/01-ELK-Stack/9b27affd9a85f2a632f4.png)

接下来创建一个新的索引模板，模板的名称为myapp，索引模式为数据流的名称，并将优先级设置为110，否则会由于优先级低于logs模板而不生效。

![](assets/01-ELK-Stack/03387f7ced46c941cd01.png)

在组件模板中我们选择继承先前自定义的模板，完全自定义创建配置。

![](assets/01-ELK-Stack/9e2725c238933114712b.png)
...
更多集群写入性能优化建议可参考文章：[08-05_%E9%9B%86%E7%BE%A4%E5%86%99%E5%85%A5%E6%80%A7%E8%83%BD%E4%BC%98%E5%8C%96.md](articles/01-ELK-Stack/08-05_集群写入性能优化.md)

更多索引设置可参考文章：[03-02_%E7%B4%A2%E5%BC%95%E5%A2%9E%E5%88%A0%E6%94%B9%E6%9F%A5%E4%B8%8E%E9%85%8D%E7%BD%AE.md](articles/01-ELK-Stack/03-02_索引增删改查与配置.md)

分片数设置可参考文章：[08-02_%E9%9B%86%E7%BE%A4%E5%AE%B9%E9%87%8F%E8%A7%84%E5%88%92.md](articles/01-ELK-Stack/08-02_集群容量规划.md)
...
![](assets/01-ELK-Stack/3ea981dd39a050a40c41.png)

**映射(mapping)**

在mapping配置中，由于我们还需要对索引字段进行调整，我们先选择动态模板，待一切调整完成后，在使用固定字段mapping。更多mapping相关文章请参考[02-02_%E7%BB%93%E6%9E%84%E5%AE%9A%E4%B9%89%EF%BC%88mapping%EF%BC%89.md](articles/01-ELK-Stack/02-02_结构定义（mapping）.md)

![](assets/01-ELK-Stack/d1cf1727663894b29d46.png)

别名设置，此处也设置为空，更多alias设置请参考[03-04_%E7%B4%A2%E5%BC%95%E5%88%AB%E5%90%8D%EF%BC%88alias%EF%BC%89.md](articles/01-ELK-Stack/03-04_索引别名（alias）.md)

![](assets/01-ELK-Stack/f4707baf27b6d6f89d94.png)

至此，索引模板创建完成

![](assets/01-ELK-Stack/c510025f91816b612c93.png)

## 创建数据流验证
我们删除原本的数据流后，会自动按索引模板设置创建新的数据流。

![](assets/01-ELK-Stack/db6b5076f0b8acdf9375.png)

查看数据流信息，已关联名为myapp-template的索引模板

![](assets/01-ELK-Stack/c1d36156b49f48b2e938.png)

查看索引设置信息，已按模板设置创建了索引，将分片数调整为3个。

![](assets/01-ELK-Stack/bafac3395ec7c500092c.png)
...
从简洁程度来说，首选方案4，如果从性能角度，首选方案2。本实例以方案4为例，演示runtime的使用。更多runtime信息请参考文档：[02-07_%E8%BF%90%E8%A1%8C%E6%97%B6%28Runtime_fields%29.md](articles/01-ELK-Stack/02-07_运行时-Runtime_fields-.md)
...
![](assets/01-ELK-Stack/74ea64bb43d65d35289b.png)

## 创建数据流验证
我们还是删除先前的数据流，让es为我们自动创建新的数据流，查看数据流信息验证

![](assets/01-ELK-Stack/555075d43b5b60ff21c4.png)
...
![](assets/01-ELK-Stack/f231b7e8a6f22a33cedc.png)
...
![](assets/01-ELK-Stack/2e819e86acf5e0e47793.png)
...
![](assets/01-ELK-Stack/c254501e945752c418af.png)

例如我们使用了<font style="color:rgb(48, 49, 51);">geoip插件获取了用户访问IP的经纬度数据，我们需要指定类型</font>为geo_point

![](assets/01-ELK-Stack/db76270a2f175638c6b9.png)

## 创建数据流验证
我们还是删除先前的数据流，让es为我们自动创建新的数据流，查看数据流信息验证，可以看到新创建的数据流字段已经按我们的期望设置了数据类型。

![](assets/01-ELK-Stack/efe872ad6e790d7f80ff.png)

# ILM
<font style="color:rgb(48, 49, 51);">随着时间的推移，索引的数据量会越来越大</font>，我们需要在索引不同的阶段，使用不同的策略配置索引，此时就需要使用ILM索引生命周期管理配置，具体可参考文章：[06-05_ILM%E7%B4%A2%E5%BC%95%E7%94%9F%E5%91%BD%E5%91%A8%E6%9C%9F%E7%AE%A1%E7%90%86.md](articles/01-ELK-Stack/06-05_ILM索引生命周期管理.md)
...
为了便于演示，我们将时间缩短，0-7分钟的数据存放hot节点，7-15分钟数据存放warm节点，15-30分钟数据存放clod节点，超过30分钟以上数据删除。

接下来我们创建一个名为myapp-policy的策略。

![](assets/01-ELK-Stack/b8e6ec869cbea80b3f78.png)

hot阶段配置如下：

![](assets/01-ELK-Stack/50954a375cfd3a59dc19.png)

warm阶段配置如下：

![](assets/01-ELK-Stack/6785a75cef021c7a3ce5.png)

cold阶段配置如下：

![](assets/01-ELK-Stack/4eb38e2dbf30e435e8c9.png)

删除阶段配置如下：

![](assets/01-ELK-Stack/b36218903e3e06f7c218.png)

## ILM与索引模板关联
创建完ILM策略后，接下来，我们将ILM策略与索引模板关联，这样后续创建的索引都会通过ILM进行管理。

![](assets/01-ELK-Stack/ebb36502793abc7e8c3e.png)
...
![](assets/01-ELK-Stack/2df4c8fac7d4d961b769.png)

hot阶段验证

0-7分钟的数据存放hot节点，且每分钟滚动生成一个索引

![](assets/01-ELK-Stack/7b386bd3861a190af952.png)

warm阶段验证

7-15分钟数据存放warm节点，合并为1分片，只读索引

![](assets/01-ELK-Stack/2f08512dd65601481f5c.png)

![](assets/01-ELK-Stack/819f8c6cd3eb770ce210.png)

clod验证

15-30分钟数据存放clod节点，调整为0副本，数据降采样

![](assets/01-ELK-Stack/dea9aa5bb8773196a2ab.png)

![](assets/01-ELK-Stack/f5c8e4e7705518e10922.png)

删除验证

超过30分钟以上数据删除。discover只能查到最近30分钟的数据

![](assets/01-ELK-Stack/ebb6556eeb913cffe581.png)

# 数据流
## 数据流简介
数据流是es7.9新推出的功能，<font style="color:rgb(48, 49, 51);">它是索引、模板、rollover、ilm 基于时序性数据的综合产物。</font>在es8中，凡是<font style="color:rgb(48, 49, 51);">日志（logs）、事件（events）、指标（metrics）和其他持续生成的数据，都强烈推荐创建数据流类型的索引。更多数据流的详细信息可参考文章：</font>[<font style="color:rgb(48, 49, 51);">02-06_%E6%95%B0%E6%8D%AE%E6%B5%81%28data_stream%29.md</font>](articles/01-ELK-Stack/02-06_数据流-data_stream-.md)
